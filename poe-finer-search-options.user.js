...
(function($) {
    ...
    const scriptStyle=$(`
        <style>
            #finer-search-global{color:white;height: 185px;width: 175px;background: #2f2f2f;position: fixed;right: 10px;top: 50px;z-index:1000}
...
//---------UTILS METHODS---------//
    const modMap={life:"total_life",cold:"total_cold_resistance",fire:"total_fire_resistance",ligt:"total_lightning_resistance",move:"increased_movement_speed"};
...
//---------ACTUAL CODE DOING STUFF AND ALL---------//
    ...
    $(document)
        ...
        .on('click','#finer-search-global > .finer-global-btn', function(e){
    ...
    const addPseudoMods = (hashes,more) => {
        ...
        hashes.forEach(hash => {
            ...
            if(currentMod){
                ...
                ISG.selectFilter(createFilter(`pseudo.pseudo_${modMap[hash]}`));
            }
        });
        window.app.save(!0);
